...
class AssessmentBankSeeder extends Seeder
{
    public function run(): void
    {
        // 1. PHI Framework
        $phi = AssessmentFramework::updateOrCreate(
            ['code' => 'PHI'],
            ['name' => 'Programme Health Index', 'is_active' => true]
        );
        ...
        foreach ($phiPillars as $code => $data) {
            $pillar = AssessmentPillar::updateOrCreate(
                ['framework_id' => $phi->id, 'code' => $code],
                ['name' => $data['name'], 'weight' => $data['weight'], 'is_critical' => $data['critical']]
            );
        }

        // Questions for PHI Snapshot
        $phiSnapshot = [
            'P1' => [
        ...
        foreach ($phiSnapshot as $pCode => $qs) {
            $pillar = AssessmentPillar::where('framework_id', $phi->id)->where('code', $pCode)->first();
            foreach ($qs as $qCode => $text) {
                AssessmentQuestionBank::updateOrCreate(
                    ['framework_id' => $phi->id, 'question_code' => $qCode],
                    ['pillar_id' => $pillar->id, 'level' => 'snapshot', 'question_text' => $text]
                );
            }
        }
        ...
        foreach ($phiFull as $pCode => $qs) {
            $pillar = AssessmentPillar::where('framework_id', $phi->id)->where('code', $pCode)->first();
            foreach ($qs as $qCode => $text) {
                AssessmentQuestionBank::updateOrCreate(
                    ['framework_id' => $phi->id, 'question_code' => $qCode],
                    ['pillar_id' => $pillar->id, 'level' => 'full', 'question_text' => $text]
                );
            }
        }

        // 2. ITSM Framework
        $itsm = AssessmentFramework::updateOrCreate(
            ['code' => 'ITSM'],
            ['name' => 'IT Service Management Health Index', 'is_active' => true]
        );
        ...
        foreach ($itsmDomains as $code => $data) {
            $pillar = AssessmentPillar::updateOrCreate(
                ['framework_id' => $itsm->id, 'code' => $code],
                ['name' => $data['name'], 'weight' => $data['weight'], 'is_critical' => $data['critical']]
            );
        }
        ...
        foreach ($itsmSnapshot as $pCode => $qs) {
            $pillar = AssessmentPillar::where('framework_id', $itsm->id)->where('code', $pCode)->first();
            foreach ($qs as $qCode => $text) {
                AssessmentQuestionBank::updateOrCreate(
                    ['framework_id' => $itsm->id, 'question_code' => $qCode],
                    ['pillar_id' => $pillar->id, 'level' => 'snapshot', 'question_text' => $text]
                );
            }
        }
