...
Status automation is now in place. Production is deployed from commit `391a14d` plus the assessment `1` PDF export hotfix, production `.env` has executable Browsershot paths and queue retry headroom, and validation assessment `1` proves real queued `pir_full_tier2` Claude generation plus real Browsershot PDF service export on the VPS. The browser-reported 500 was fixed by using unique PDF temp paths and making the PIR score page full-report-schema tolerant.

Next safest action: retry the normal production browser Export PDF button on assessment `1`; if it downloads, visually confirm the PDF is non-blank and includes Tier 2 stakeholder intelligence and evidence validation sections. After that, create/score/generate/export a second PIR Tier 2 assessment through the admin UI only if owner-level workflow proof is required beyond the controlled server validation.
