...
Status automation is now in place. Production is deployed from commit `391a14d` plus the assessment `1` PDF export hotfix, production `.env` has executable Browsershot paths and queue retry headroom, and validation assessment `1` proves real queued `pir_full_tier2` Claude generation plus real Browsershot PDF service export on the VPS. Local completed PIR test assessments are now available in production as assessment `2` (Tier 1) and assessment `3` (Tier 2), each with imported evidence, pillar scores, stored AI draft data, and validated server-side PDF export.

Next safest action: open production assessments `2` and `3` in the admin browser, inspect the imported preview/evidence data, click the normal Export PDF button for both, and visually confirm the PDFs are non-blank and include the expected Tier 1/Tier 2 sections. After that, create/score/generate/export a fresh PIR Tier 2 assessment through the admin UI only if owner-level workflow proof is required beyond the controlled server validation and imported test records.
